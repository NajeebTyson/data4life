...
func (s *TokenStore) AddToken(t *token.Token) error {
	sqlStatement := `
...
func (s *TokenStore) AddTokenBatch(tokens []string) error {
...
func (s *TokenStore) GetToken(t string) (*token.Token, error) {
...
func (s *TokenStore) DeleteToken(t string) error {
	sqlStatement := `
		DELETE FROM tokens
		WHERE token = $1;
	`
